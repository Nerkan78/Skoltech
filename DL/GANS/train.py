...
class GAN(pl.LightningModule):
    def __init__(
        ...
        if self.truncation_trick:
            ...
            self.register_buffer('val_noise', torch.from_numpy(stats.truncnorm.rvs(-1, 1, size=(len(self.train_dataset), self.noise_channels))).float(), persistent=False)
    ...
    def training_step(self, batch, batch_idx, optimizer_idx):
        ...
        noise = torch.randn(len(imgs), self.noise_channels).cuda()
